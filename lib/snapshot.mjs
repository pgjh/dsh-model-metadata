 ...
 *
 * This module also owns the two *locations* — which file, which endpoint — for
 * the same reason: they used to be spelled out again in `lib/index.mjs` and in
 * the refresh CLI, and the installer hardcoded a third copy of the file name. A
 * manual refresh then wrote a snapshot the running plugin never read, and a
 * purge deleted a file it was not using, whenever
 * `DSH_PI_AI_CATALOG_SNAPSHOT` pointed the runtime somewhere else.
 *
...
/**
 * The endpoint the snapshot is fetched from: the override, else models.dev.
 *
 * Read at call time rather than captured at import time, because a tool that
 * offers `--url` (or a test that sets the variable) must be able to point the
 * fetch at a mirror without the environment being settled before the module
 * graph loads.
 * @returns the endpoint to read.
 */
export function snapshotUrl() {
	const configured = process.env.DSH_PI_AI_CATALOG_SNAPSHOT_URL;
	return configured !== undefined && configured.length > 0 ? configured : SNAPSHOT_URL;
}

/**
 * The file the snapshot lives in, inside a given DSH home: the override, else
 * the default name there.
 *
 * `home` is a parameter rather than a lookup so the one installer that also
 * accepts `--home` can ask about the home it is actually writing to instead of
 * the one the environment happens to name.
 * @param home - the DSH home directory.
 * @returns the snapshot path.
 */
export function snapshotDefaultPath(home) {
	const configured = process.env.DSH_PI_AI_CATALOG_SNAPSHOT;
	return configured !== undefined && configured.length > 0 ? configured : join(home, "models-dev-snapshot.json");
}
