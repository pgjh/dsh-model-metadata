 ...
 * @param only - answer just this route, when given. Applied *before* inspecting:
 * one card asks for one route, and looking up the verdicts of every other route's
 * models first is work that is then thrown away.
 * @returns the routes, in stored order, each with its models.
 */
export function buildMatrix(providers, inspect, only = undefined) {
	const routes = [];
	for (const [route, profile] of Object.entries(providers ?? {})) {
		...
		for (const entry of profile.models ?? []) {
			...
			models.push({
				...
				...Array.isArray(view.nearby) && view.nearby.length > 0 ? { nearby: view.nearby.slice(0, 5) } : {},
