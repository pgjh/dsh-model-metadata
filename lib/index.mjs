 ...
 *   1. the model family's own upstream route (glm -> zai, kimi -> moonshotai, …)
 *   2. opencode / opencode-go
 *   3. the rest of the installed pi-ai catalog, in catalog order
...
/** `bundled` takes image support only from the shipped catalog, never from models.dev. */
...
/**
 * The by-name index over the installed catalog plus the snapshot.
 *
 * Memoized by the snapshot's mtime, not forever: `refresh-snapshot.mjs` (or the
 * background refresh below) rewrites the file, and the very next model
 * resolution must see the new data without a restart. The installed catalog
 * cannot change while the process lives, so it is not part of the key.
 * @returns the lookup used by every resolution.
 */
let indexCache;
function fallbackIndex() {
	const snapshot = snapshotState();
	const revision = `${String(snapshot.stamp)}:${String(snapshot.size)}`;
	...
	indexCache = { revision, index: { byName, byLower, routeOrder, exact } };
...
function matchFor(id) {
	...
	const candidates = index.byName.get(bare) ?? index.byLower.get(bare.toLowerCase());
...
/**
 * The parsed settings document, re-read whenever the file changes (mtime *and*
 * size, for filesystems with coarse timestamps). Undefined when it is missing,
 * unreadable, or has no YAML parser available — never a failure.
 */
let settingsCache;
function settingsDocument() {
	const yaml = yamlLoad.exports;
	if (yaml === undefined || typeof yaml.load !== "function") return undefined;
...
function enrichModel(model, provider) {
	...
	if (MATCH_INPUT && (!INPUT_BUNDLED_ONLY || matched.source === "pi-ai-catalog") && declared.input === undefined && Array.isArray(matched.input) && matched.input.length > 0) {
