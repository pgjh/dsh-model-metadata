...
export function inspect(id) {
	 ...
	 * one- or two-letter prefix would drown the suggestion in noise. Capped at
	 * five, best-ranked first.
	 */
		...
		for (const head of [words[0], words[1]]) {
			...
			for (const [key, list] of index.byNorm) {
				...
				for (const candidate of list) {
					if (candidate.contextWindow === undefined || hits.some((hit) => hit.bare === candidate.bare && hit.route === candidate.route)) continue;
					hits.push(candidate);
				}
			}
			if (hits.length === 0) continue;
			return hits
				.sort((left, right) => rank(index, bare, left) - rank(index, bare, right))
				.slice(0, 5)
