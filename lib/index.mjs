...
import { SNAPSHOT_URL, fetchModelsDevSnapshotIfChanged } from "./snapshot.mjs";
 ...
 * packages somewhere neither the running binary nor the DSH home implies.
...
function fallbackIndex() {
	...
	indexCache = { revision, index: { revision, byName, byLower, byNorm, routeOrder, exact, firstPartyCount: firstParty.length } };
 ...
 * two controls in the editor then show a short "not available" line instead of
 * values, and the metadata fallback is untouched), and
 * `DSH_PI_AI_CATALOG_PANEL_HOSTS=<list>` narrows it to the names it is served
 * under.
...
export function inspect(id) {
	 ...
	 *
	 * Bounded on purpose. A generic head (`kimi`, `gemini`, `gpt`) matches hundreds
	 * of catalog rows across the whole normalized index, and collecting every one of
	 * them to show three was both the most expensive and the least useful part of a
	 * panel request: the identity guard and the scan cap keep the work proportional
	 * to what the caller asked for (five suggestions), not to the catalog's size.
	 */
	...
	const nearby = ranked.length === 0 ? (() => {
		...
		for (const head of [words[0], words[1]]) {
			...
			const seen = new Set();
			const hits = [];
			for (const [key, list] of index.byNorm) {
			...
			return rankCandidates(index, bare, hits.slice(0, NEARBY_SCAN_CAP))
				.slice(0, NEARBY_LIMIT)
				.map((candidate) => ({ id: candidate.bare, route: candidate.route, source: candidate.source, contextWindow: candidate.contextWindow }));
		}
		return [];
	})() : undefined;
	return {
...
function install(proto, logger) {
	 ...
	 * value — and it fills the same three fields the primary path does, capacities
	 * aside: a build that needed this path would otherwise report image support and
	 * output caps exactly as the seam left them, which is the whole defect this
