...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const en = {
			...
			"readout.noMatch": "No match: no catalog knows this name — set the capacity under Capacity",
			"readout.listSeparator": ", ",
		 ...
		 * reads it — and how the shipped 容量 placeholder spells its own defaults. The
		 * approximate spelling is the one form that is not language-neutral, so it comes
		 * from the dictionary: `约 123K` and `~123K` are the same number, each spelled the
		 * way its own language reads an approximation.
		...
		function formatCapacity(value) {
			...
			return value >= 1000 ? text("capacity.approx", { value: `${String(Math.round(value / 1000))}K` }) : String(value);
		}
		...
		/**
		 * One row's declaration-derived choice, remembered per row object.
		 *
		 * `Cell` is memoized and the memo compares its `choice` prop by identity, so a
		 * fresh object per render is exactly what used to re-render every cell of the card
		 * on every keystroke. The row object is the right key for the cache: the matrix
		 * payload — and with it the rows — is rebuilt precisely when the declarations may
		 * have changed, so the cache invalidates itself and can never serve a stale
		 * declaration.
		 * @returns a `(row) => { vision, levels }` answering with one object per row.
		 */
		function chooseByRow() {
			const seen = new WeakMap();
			return (row) => {
				const cached = seen.get(row);
				if (cached !== undefined) return cached;
				const derived = initialChoice(row);
				seen.set(row, derived);
				return derived;
			};
		}
		...
		function readoutOf(row) {
			...
			if (row.matched === undefined) {
				...
				if (Array.isArray(row.nearby) && row.nearby.length > 0) {
					...
					/* The separator is the dictionary's: a comma between English names, the
					 * enumeration mark between Chinese ones. */
					parts.push(text("readout.siblings", { names: names.slice(0, 3).join(text("readout.listSeparator")), more: names.length > 3 ? text("readout.more") : "" }));
				}
			} else {
		...
		function Panel(props) {
			...
			/*
			 * The id inputs the last `read()` saw. A row's id is the one thing that can
			 * change without a childList mutation (React writes it through the input's
			 * `value` property), so these are the only inputs worth waking the scan for.
			 */
			const idInputs = React.useRef(null);
			...
			/*
			 * The choice a row derives from its own declaration, cached per row object.
			 *
			 * `initialChoice` builds a fresh object on every call, and rows without a
			 * pending edit all take that path — so a new object per render for every row
			 * meant `before.choice === after.choice` in the cell's memo comparator was
			 * never true, and one keystroke re-rendered every cell of the card, which is
			 * the exact cost the memo exists to remove.
			 *
			 * The cache is a WeakMap held in a ref, and it needs no invalidation of its
			 * own: `rows` is memoized on the payload, so a row object is rebuilt only when
			 * the matrix changes (a refetch, or the patch a write applies), and a rebuilt
			 * row is a new key with a freshly derived choice. An old row — and its entry —
			 * is then unreachable and collectable. The invariant the cell relies on: as
			 * long as a row object stays the same, so does the choice object derived from
			 * it, so a render that changed nothing else cannot reach a cell.
			 */
			/* Built once per Panel: a fresh WeakMap per render would defeat the memo, and a
			 * module-level one would outlive the payload it describes. */
			const derivations = React.useRef(undefined);
			if (derivations.current === undefined) derivations.current = chooseByRow();
			const derivedChoice = derivations.current;

			const choiceOf = (row) => choices[row.id] ?? derivedChoice(row);

			const change = React.useCallback((id, patch) => {
				setChoices((current) => {
					const row = rows.find((entry) => entry.id === id);
					if (row === undefined) return current;
					return { ...current, [id]: { ...(current[id] ?? derivedChoice(row)), ...patch } };
				});
			 ...
			 *
			 * That listener ignores everything except the id inputs `read()` itself
			 * visited. It used to take any keystroke on the card, and every one of them
			 * bought a `querySelectorAll` over the whole card plus a scan of the rows,
			 * for an edit (a name, a capacity digit) that cannot change which rows exist.
			 */
			React.useEffect(() => {
				...
				const read = () => {
					...
					for (const toggle of rowToggles(card)) {
						...
						/* Every id input is remembered, empty ones included: a row whose id has
						 * not been typed yet is exactly the one whose next keystroke has to reach
						 * `read()` — a row that is not a row *yet* is still a row to watch. */
						if (idInput !== undefined && idInput !== null) inputs.add(idInput);
						if (entry === undefined || entry === null || typeof id !== "string" || id.length === 0) continue;
						/* Keyed by id, not by comparing every row with every earlier one: two
						 * rows claiming one id would portal two cells into the first of them. */
						if (seen.has(id)) continue;
						seen.add(id);
						found.push({ entry, id });
					}
					idInputs.current = inputs;
				...
				/*
				 * Capture phase, so an id typed into a row reaches this before anything can
				 * stop it. The set is `read()`'s: only the id inputs it just visited are
				 * worth a rescan, and it runs once below, before any keystroke can arrive.
				 */
				const onInput = (event) => {
					if (idInputs.current?.has(event.target) !== true) return;
					schedule();
				};
