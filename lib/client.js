...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function readoutOf(row) {
			const declared = row.declared ?? {};
			const parts = [];
			if (row.matched === undefined) parts.push("无匹配：两处目录都不认识这个名字，容量请在「容量」里填");
			else {
		...
		const styles = {
			...
			/* The trigger keeps the row input's box; the caret is DSH's own icon. */
			trigger: { boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "6px", width: "100%", textAlign: "left", cursor: "pointer" },
		...
		function Panel(props) {
			...
			const classes = React.useMemo(() => {
				...
				const extra = select === undefined || select === null || select === sample ? "" : select.className;
				...
				return {
					control: `${sample === undefined || sample === null ? "" : sample.className} ${extra}`.trim(),
